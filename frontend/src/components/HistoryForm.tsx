  ...
  if (loading) {
    return (
      <div className="w-full h-[600px] flex flex-col items-center justify-center gap-6 font-['Cinzel']">
        <div className="w-16 h-16 border-2 border-[#d4af37]/10 border-t-[#d4af37] rounded-full animate-spin"></div>
        <p className="text-[#d4af37] text-[10px] tracking-[0.6em] uppercase font-bold animate-pulse">Sincronizando archivos...</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto h-[85vh] flex flex-col font-['Cinzel'] overflow-hidden">
      ...
      <div className="flex flex-col items-center shrink-0 pt-10 px-6">
        <h2 className="text-4xl md:text-5xl font-black text-white tracking-[0.2em] uppercase mb-2 text-center">
          Historial
        </h2>
        <div className="text-[#d4af37]/60 text-[10px] tracking-[0.4em] uppercase font-bold mb-8">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-8 custom-scrollbar space-y-3">
        {filteredGames.map((game) => {
          ...
          return (
            <div 
              key={game.id}
              className={`w-full group relative flex items-center justify-between p-4 md:p-6 border ${style.border} ${style.bg} ${style.shadow} rounded-2xl transition-all duration-300 hover:scale-[1.01] hover:border-[#d4af37]/40`}
            >
              <div className="flex items-center gap-6">
                <div className="hidden md:flex flex-col items-center justify-center w-14 h-14 rounded-xl bg-black/40 border border-white/5 font-black">
                    <span className="text-[#d4af37] text-[10px] uppercase">{game.timeControl}</span>
                    <span className="text-zinc-500 text-[8px] uppercase">{game.mode}</span>
                </div>
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {game.eloChange && game.result !== 'draw' && (
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded bg-white/5 ${game.result === 'win' ? 'text-emerald-400' : 'text-rose-500'}`}>
                            {game.eloChange} ELO
                        </span>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-8">
                <div className="hidden sm:block text-right">
                  ...
                  <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-[0.1em]">{game.moves} JUGADAS</p>
                </div>
                <button className="h-10 w-10 md:h-12 md:w-32 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-[#d4af37] hover:text-black transition-all group/btn font-black text-[10px] uppercase tracking-widest cursor-pointer">
