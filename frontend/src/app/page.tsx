          ...
          <section className="p-8 md:p-12 max-w-[1400px] mx-auto w-full pt-8 pb-16"> 
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
              ...
              <SectionBox title="Desafío Táctico">
                <div className="flex gap-6 items-center flex-grow bg-[#d4af37]/5 rounded-2xl p-6 border border-[#d4af37]/10">
                  <div className="w-24 h-24 rounded-lg bg-black/40 border border-[#d4af37]/20 flex items-center justify-center text-5xl">🧩</div>
                  <div className="flex-grow">
                    <p className="text-[11px] text-[#d4af37]/70 font-black uppercase tracking-widest">Dificultad Media</p>
                    <h4 className="text-xl font-bold text-white uppercase tracking-tight mt-1 font-['Cinzel']">Ganan Blancas</h4>
                    <p className="text-xs text-zinc-400 mt-1">Encuentra la secuencia de mate forzado.</p>
                  </div>
                  <Link href="/puzzles" className="px-6 py-3 bg-[#d4af37] text-black rounded-full font-bold text-xs uppercase tracking-widest hover:bg-white transition-colors shadow-lg active:scale-95">
                    Resolver
                  </Link>
                </div>
              </SectionBox>
...
function GameCard({ href, title, subtitle, desc, stats, img, online = false }: any) {
  return (
    <Link href={href} className="group relative h-64 rounded-3xl border border-[#d4af37]/20 bg-black/40 p-8 overflow-hidden flex flex-col justify-between hover:border-[#d4af37] hover:shadow-[0_0_50px_rgba(212,175,55,0.2)] hover:-translate-y-2 transition-all duration-500 shadow-xl">
      ...
      <div className="relative z-10">
        ...
        <h3 className="text-3xl font-bold text-white uppercase tracking-tighter leading-tight group-hover:text-[#d4af37] transition-colors font-['Cinzel']">{title}</h3>
      </div>
      <div className="relative z-10 bg-black/50 backdrop-blur-sm p-4 rounded-xl border border-white/5 mt-auto">
        <p className="text-sm text-zinc-300">{desc}</p>
        <span className="text-[10px] text-zinc-500 uppercase font-bold mt-2 block tracking-widest">{stats}</span>
      </div>
    </Link>
  );
}
...
function SectionBox({ title, children }: any) {
  return (
    <div className="bg-black/50 border border-[#d4af37]/15 rounded-3xl p-8 relative overflow-hidden shadow-2xl transition-all duration-500 hover:border-[#d4af37]/40 group">
      <div className="relative z-10">
...
function StatBox({ label, value, accent = false }: any) {
  return (
    <div className="bg-white/5 rounded-xl p-3 border border-white/5">
      <span className="block text-[10px] text-zinc-500 uppercase font-bold tracking-tighter">{label}</span>
      <span className={`text-xl font-light italic leading-none ${accent ? 'text-[#d4af37]' : 'text-white'} font-['Cinzel']`}>{value}</span>
    </div>
  );
}
...
function ActivityBars() {
  return (
    <div className="h-24 w-full bg-black/40 rounded-xl border border-white/5 p-4 flex items-end gap-1 group/bars">
